...
def rotate_and_scale_by_matrix(rotation, scale, vector, input_origin, output_origin):
    """
        Rotates and scales a vector (a position in 3D).

        The vector is first shifted to `input_origin`,
            then it is rotated and scaled by the given matrix and scale coefficient,
        ...
        Args:
            rotation (list of list of float): 2D rotation matrix; a list of two
                lists of two numbers
    ...
    """
    in_x = vector[0] - input_origin[0]
    in_y = vector[1] - input_origin[1]
    out_x = scale * (rotation[0][0]*in_x + rotation[0][1]*in_y) + output_origin[0]
    out_y = scale * (rotation[1][0]*in_x + rotation[1][1]*in_y) + output_origin[1]
    return [out_x, out_y, vector[2]]

def rotate_and_scale_by_quaternion(rotation, scale, vector, input_origin, output_origin):
    """
        Rotates and scales a vector (a position in 3D).
...
class NMEAParser:
    ...
    def parse(self, data):
        ...
        if self.converter:
            ...
            self.converter = Convertor((lon, lat))
...
class LeastSquaresLocalization:
    """
    ...
    def compute_post_process_trajectory(self):
        ...
        if abs(last.tra - first.tra) > self.post_window:
            ...
            for k in range(first_index, last_index + 1):
                ...
                new_xyz = rotate_and_scale_by_quaternion(qua, sca, s.odo, or_o, or_g)
        ...
        if len_trajectory == 0:
            ...
            if abs(last.tra - first.tra) > self.window:
                ...
                if all(var is not None for var in [qua_est, sca_est, self.init_qua, self.init_sca]):
                    ...
                    pose3d_xyz_init = rotate_and_scale_by_quaternion(self.init_qua, self.init_sca, last.odo, first.odo, first.gps)
                    pose3d_ori_init = quaternion.multiply(self.init_qua, last.ori)
                    self.plot_init.append(pose3d_xyz_init)
                    pose3d_xyz_est = rotate_and_scale_by_quaternion(qua_est, sca_est, last.odo, first.odo, first.gps)
                    ...
                    pose3d_xyz = rotate_and_scale_by_quaternion(self.init_qua, self.init_sca, last.odo, first.odo, first.gps)
                    pose3d_ori = quaternion.multiply(self.init_qua, last.ori)
                    self.pose3d = [pose3d_xyz, pose3d_ori]
                elif all(var is not None for var in [qua_est, sca_est]):
                    pose3d_xyz = rotate_and_scale_by_quaternion(qua_est, sca_est, last.odo, first.odo, first.gps)
            ...
            rot, qua, sca = compute_rotation_and_scale(self.sync_gps_odo,
                                                       ...
                                                       prune = self.prune)
            new_xyz = rotate_and_scale_by_quaternion(qua, sca, last.odo, or_o, or_g)
